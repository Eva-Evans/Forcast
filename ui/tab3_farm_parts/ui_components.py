...
def render_tab3_upload_only() -> None:
    """Загрузка данных и статус подразделений (без старого прогноза по хозяйствам)."""
    st.subheader("Загрузка данных по подразделениям")
    _ensure_farm_tables()
    ...
    if sub_status_df.empty:
        st.info("В БД пока нет подразделений. Загрузите файлы ниже.")
    else:
        with st.expander("Состав хозяйств (подразделения)", expanded=True):
    ...
    with st.expander("Добавить/обновить подразделения хозяйства в БД", expanded=True):
        mode = st.radio(
            "Режим записи",
        ...
        files = st.file_uploader(
            "Файлы по подразделениям: отёлы, осеменения, запуск, выбытие, быки — отдельно или в любых сочетаниях "
            "(один Excel с колонкой «Событие» тоже подходит)",
            type=["xls", "xlsx", "txt"],
            accept_multiple_files=True,
            key="tab3_farm_files_upload",
        )
        st.caption(
            "Общие файлы на все подразделения поддерживаются: в Excel должны быть колонки хозяйства и подразделения "
            "(Source.Name / Столбец1). На вкладке «Прогноз» выберите нужное подразделение."
        )
        ...
        if isinstance(detect_df, pd.DataFrame) and not detect_df.empty:
            st.markdown("**Распознанные файлы**")
        ...
        if not ready_upload_subs:
            st.warning("Нет ни одного подразделения с полным комплектом файлов.")
            return

        upload_log_box = st.empty()

        if st.button("Загрузить в БД", key="tab3_btn_upload_db_only", use_container_width=True):
            ...
            _log(f"Готовых комплектов: {len(ready_upload_subs)}")
            ...
            for i, subdivision in enumerate(ready_upload_subs, start=1):
                try:
                    ...
                    _log(f"{subdivision}: OK")
                except Exception as e:
                    errors.append(f"{subdivision}: {e}")
                    _log(f"{subdivision}: {e}")
                prog.progress(i / max(1, len(ready_upload_subs)))
            prog.empty()
            if errors:
                st.session_state["tab3_last_upload_status"] = {"kind": "error", "message": "Часть подразделений не загрузилась."}
                st.error("Ошибки:\n- " + "\n- ".join(errors))
            else:
...
def render_tab3_farm() -> None:
    from config import SHOW_TAB3_LEGACY_FARM_FORECAST

    if not SHOW_TAB3_LEGACY_FARM_FORECAST:
        render_tab3_upload_only()
        return
